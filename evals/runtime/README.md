...
This harness compares `baseline`, `static-full`, `adaptive-fixed-resolution`,
and `adaptive-end-to-end` under matched task, model, and generation settings.
`adaptive-runtime` is a compatibility alias for end-to-end. Core tests use only
the built-in mock or fake transports: no API key, paid call, model download, or
network access is performed automatically.
...
experimental protocol and exact live-adapter dry-run commands.
